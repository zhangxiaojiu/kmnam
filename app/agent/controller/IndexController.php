...
class IndexController extends UserBaseController
{
	...
	$list = Db::name('agentFollow')->where(['user_id'=>$user['id'],'agent_id'=>$info['id']])->order('id desc')->paginate(5);
	$this->assign('list',$list);
	...
	if($info){
	    ...
	    // 一次读取一列
	    $res = array();
	    $agent = [0=>'name',1=>'tel',2=>'wechat',3=>'address',4=>'company',5=>'brand',6=>'remark'];
